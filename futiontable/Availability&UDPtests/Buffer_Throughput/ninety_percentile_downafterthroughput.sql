DELIMITER //
DROP PROCEDURE IF EXISTS ninety_percentile_downafterthroughput_3g//
CREATE PROCEDURE ninety_percentile_downafterthroughput_3g()
BEGIN
...
getdata : LOOP
	...
	SELECT afterthroughput  INTO minthr from down_throughput WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and beforepackets > 0 AND bitrate > 3999  AND nodeisp= node_isp order by afterthroughput limit lowlimit,1 ;
	SELECT afterthroughput INTO maxthr from down_throughput WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and beforepackets > 0 AND bitrate > 3999  AND nodeisp= node_isp order by afterthroughput limit uplimit,1 ;
	SELECT afterthroughput INTO medianthr from down_throughput WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and beforepackets > 0 AND bitrate > 3999  AND nodeisp= node_isp order by afterthroughput limit medianlimit,1 ;
	SELECT nodeisp,n.district, i.name,count(*),Round(avg(afterthroughput),3) as avgsize,Round(min(afterthroughput),3) as mnsize,Round(max(afterthroughput),3) as mxsize, Round(std(afterthroughput),3) as stdsize, Round(medianthr,3) as mediantr from down_throughput as u,test.node_isp as ni, test.nodes as n , test.isp as i WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and afterthroughput between minthr and maxthr AND bitrate > 3999  AND nodeisp= node_isp AND ni.MeasurementNode_id = n.id and ni.ISP_id = i.id and ni.id=u.nodeisp;
END LOOP getdata;
CLOSE nodeisps;
END //
DELIMITER ;
...
DELIMITER //
DROP PROCEDURE IF EXISTS ninety_percentile_downafterthroughput_2g//
CREATE PROCEDURE ninety_percentile_downafterthroughput_2g()
BEGIN
DECLARE cnt  INT DEFAULT 0;
DECLARE node_isp INT DEFAULT 0;
DECLARE minthr  FLOAT DEFAULT 0.0;
...
getdata : LOOP
	...
	SET uplimit = 0.9 * cnt;
	SET medianlimit = 0.5 * cnt;
	SELECT afterthroughput  INTO minthr from down_throughput WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and beforepackets > 0   AND bitrate between 2000 and 3900  AND nodeisp= node_isp order by afterthroughput limit lowlimit,1 ;
	SELECT afterthroughput INTO maxthr from down_throughput WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and beforepackets > 0   AND bitrate between 2000 and 3900  AND nodeisp= node_isp order by afterthroughput limit uplimit,1 ;
	SELECT afterthroughput INTO medianthr from down_throughput WHERE afterthroughput > 0 and afterpackets > 1 and beforepackets >1  and beforethroughput > 0 and beforepackets > 0   AND bitrate between 2000 and 3900  AND nodeisp= node_isp order by afterthroughput limit medianlimit,1 ;
...
call ninety_percentile_downafterthroughput_3g()
